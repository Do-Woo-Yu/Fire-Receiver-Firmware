...
byte UP_BF_key = 0, UP_BFBF_key = 0, UP_Key_Value = 0,Time_1Sec_test = 0;
...
extern unsigned char TEVA_TECH_TEXT_Send[26]; // 복구시 홈화면 테바테크 텍스트 출력
...
/** USART1 **/
byte test_index = 0,test_rxbuf[255];
unsigned int Rcv1_cnt = 0, Tx1_send_number = 0,Tx1_index = 0;
unsigned char Fire_Detector_buffer[8] = {0x01,0x04,0x00,0x00,0x00,0x01,0x31,0xCA}; // 실제 전송할 데이터 Buffer
word CheckSum_data = 0;
byte Fire_Detector_Num_Cnt = 0,list_cnt = 0, detector_tx_cnt = 0;
byte Fire_Detector_List[200];
byte Fire_Tx_Cnt = 0;
unsigned int T100ms_cnt = 0; 


/** CRC16 값 test용 **/
/*byte crc16_cnt = 0x01,crc16_num = 0;
byte Tx_buffer[5] = {0xC9,0x04,0x02,0x00,0x01};
byte CRC16_Buffer_High_byte[250];
byte c9_crc_16_cnt = 0;*/
/* USER CODE END PD */

/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
void LCD_Tx(byte lcd_tx_num);
void Fire_Detector_Tx(byte detector_tx_num);
/* USER CODE END PM */
...
void Fire_Detector_Tx(byte detector_tx_num)	//UART1
{
	/*Send the character*/
	USART1->DR = Fire_Detector_buffer[0];
	Tx1_index = 1;
	Tx1_send_number = detector_tx_num-1;
	USART1->CR1 &= ~USART_CR1_RXNEIE; 	//USART1's RXE Interrupt Disable	
...
void TIM2_IRQHandler(void)
{
  ...
  if(++T10ms_cnt >= 100)
  {
	...
	/** 자동 복구 스위치 **/
	Automatic_recovery_SW = !((GPIOE -> IDR & GPIO_PIN_11) >> 11);
	...
	if(Automatic_recovery_Key_Value == 1) // 자동복구 SW가 눌려있다면
	{
		...
		if((TB1_Fire_Fg || TB2_Fire_Fg || TB3_Fire_Fg || TB4_Fire_Fg || TB5_Fire_Fg || TB6_Fire_Fg || TB7_Fire_Fg || TB8_Fire_Fg || TB9_Fire_Fg || TB10_Fire_Fg) && TB_Fire_Cnt == 0)
		{
		  if(TB_IN_AD1 >= 2.0 && TB_IN_AD2 >= 2.0 && TB_IN_AD3 >= 2.0 && TB_IN_AD4 >= 2.0 && TB_IN_AD5 >= 2.0 && TB_IN_AD6 >= 2.0 && TB_IN_AD7 >= 2.0 && TB_IN_AD8 >= 2.0 && TB_IN_AD9 >= 2.0 && TB_IN_AD10 >= 2.0 && TB_Fire_Cnt == 0)
		  {
			  ...
			  memcpy(Tx_Data_List[List_Cnt], TEVA_TECH_TEXT_Send, 26); // 청주대학교 텍스트 출력
			  List_Cnt++;
			  ...
			  list_cnt = 0;
			  detector_tx_cnt = 0;
			  memset(Fire_Detector_List, '\0', 250); 
  ...
  if(Fire_Tx_Cnt)
  {
	  ...
	  {
		  T100ms_cnt = 0;
		  if(Fire_Detector_List[detector_tx_cnt] != 0x00)
		  {
				//unsigned char Fire_Detector_buffer[8] = {0x01,0x04,0x00,0x00,0x00,0x01,0x31,0xCA}; // 실제 전송할 데이터 Buffer
					
				Fire_Detector_buffer[0] = Fire_Detector_List[detector_tx_cnt];
					
				//CRC(Hex)
				CheckSum_data = CRC16(Fire_Detector_buffer, 6);	//송신할 데이터의 CheckSum계산 
					
				Fire_Detector_buffer[6] = CheckSum_data & 0xFF;	    //CRC's Low byte	
					
				Fire_Detector_buffer[7] = (CheckSum_data >> 8);		//CRC's High byte	
				
				Fire_Detector_Tx(8); // 감지기로 데이터 전송
  ...
  /*if(crc16_cnt <= 0xC8)
  {
	  ...
	  {
		  T100ms_cnt = 0;
		  Tx_buffer[3] = crc16_cnt++; 
		  CheckSum_data = (CRC16(Tx_buffer, 5) >> 8);	//송신할 데이터의 CheckSum High Byte 계산 
		  CRC16_Buffer_High_byte[crc16_num++] = CheckSum_data;
		  if(CRC16_Buffer_High_byte[crc16_num - 1] == 0xC9)
		  {
			  c9_crc_16_cnt++;
		  }
	  }
...
void TIM3_IRQHandler(void)
{
  ...
  static unsigned int CHK_BAT_Check_Fg = 0;
  ...
  {  
	...
	if(TB_IN_AD1 <= 1.0 && TB1_Fire_Fg == 0) // 회로1 화재
	{
	  if(++TB1_Fire_Cnt >= 1000) // 1초간 회로1 화재 신호가 들어오면 실행 
	  {
		...
		Fire_Detector_Num_Cnt = 1; // 회로 1번에 연결되어있는 감지기는 1번 ~ 20번까지임
		
		for(byte i = list_cnt; i < list_cnt + 20; i++)
		{
			Fire_Detector_List[i] = Fire_Detector_Num_Cnt;
			Fire_Detector_Num_Cnt++; // 하나씩 증가시키면서 감지기 번호를 리스트에 넣어서 데이터를 전송할 준비를 한다.
		}
		list_cnt += 20;
	...
	if(TB_IN_AD2 <= 1.0 && TB2_Fire_Fg == 0) // 회로2 화재
	{
	  if(++TB2_Fire_Cnt >= 1000) // 1초간 회로2 화재 신호가 들어오면 실행 
	  {
		...
		Fire_Detector_Num_Cnt = 21; // 회로 2번에 연결되어있는 감지기는 21번 ~ 40번까지임
	...
	if(TB_IN_AD3 <= 1.0 && TB3_Fire_Fg == 0) // 회로3 화재
	{
	  if(++TB3_Fire_Cnt >= 1000) // 1초간 회로3 화재 신호가 들어오면 실행 
	  {
		...
		Fire_Detector_Num_Cnt = 41; // 회로 3번에 연결되어있는 감지기는 41번 ~ 60번까지임
	...
	if(TB_IN_AD4 <= 1.0 && TB4_Fire_Fg == 0) // 회로4 화재
	{
	  if(++TB4_Fire_Cnt >= 1000) // 1초간 회로4 화재 신호가 들어오면 실행 
	  {
		...
		Fire_Detector_Num_Cnt = 61; // 회로 4번에 연결되어있는 감지기는 61번 ~ 80번까지임
	...
	if(TB_IN_AD5 <= 1.0 && TB5_Fire_Fg == 0) // 회로5 화재
	{
	  if(++TB5_Fire_Cnt >= 1000) // 1초간 회로5 화재 신호가 들어오면 실행 
	  {
		...
		Fire_Detector_Num_Cnt = 81; // 회로 5번에 연결되어있는 감지기는 81번 ~ 100번까지임
...
word CheckSum_data;
byte Rx1_step = 0;
byte Rx1_CRC_H = 0, Rx1_CRC_L = 0;
byte CheckSum_ing = 0;
byte CheckSum_EN = 0;
byte last_equip_num = 0;
word Fire_Detector_Data_crc = 0, P_Type_Reciever_Data_crc = 0;
byte rx1_data = 0;
void USART1_IRQHandler(void)
{
  /* USER CODE BEGIN USART1_IRQn 0 */
	...
	byte rx_test_cnt = 0; 
	if((USART1->CR1 & USART_CR1_RXNEIE) && (USART1-> SR & USART_SR_RXNE))	// 수신버퍼가 채워졌다면?	
	{
	  Rcv1_cnt++;
	  rx1_data = USART1->DR;	// 수신값 저장
	  ...
	  if(CheckSum_ing == 0)
	  {
			...
			{
				case 0:
					if(rx1_data == 0xC9) // 감지기에서 온 데이터 암호가 (0xC9(201))가 동일한가?
					{
						Rx1_buf[0] = rx1_data;
						Rx1_step = 1;
						Rx1_index = 1;
				...
				case 2: // Master에서 읽기 요청한 데이터 수
					Rx1_buf[Rx1_index++] = rx1_data;
					Rx1_step++;
					break;	
				
				case 3: // 레지스터 데이터( 장비 번호 데이터 )
					Rx1_buf[Rx1_index++] = rx1_data;
					Rx1_step++;
					break;
					
				case 4: // 레지스터 데이터( 감지기 화재 감지 데이터 0x01이면 화재 0x00이면 화재X )
					Rx1_buf[Rx1_index++] = rx1_data;
					Rx1_step++;
					break;
					
				case 5: //Waiting for CRC16 (Low byte)	
					Rx1_buf[Rx1_index++] = rx1_data;
					Rx1_step++;
					break;

				case 6: //Waiting for CRC16 (High byte)	
					Rx1_buf[Rx1_index] = rx1_data;	
					CheckSum_EN = 1;
					CheckSum_ing = 1;
	...
	else if((USART1->CR1 & USART_CR1_TCIE) && (USART1->SR & USART_SR_TC))
	{
	  static byte Tx1_cnt=0;
	  Tx1_cnt++;
	  if(Tx1_send_number)
	  {
		  Tx1_send_number--;  
		  USART1->DR = Fire_Detector_buffer[Tx1_index++];
	  }
	  else	//모든 Data 전송 하였음.
	  {
		  USART1->CR1 |= USART_CR1_RXNEIE; 	// UART1's RXE Interrupt Enable		
		  USART1->CR1 &= ~USART_CR1_TCIE;		// UART1's TXE Interrupt Disable
	  }
	  return;	
	}	
